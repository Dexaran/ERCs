...
[ERC-223](./eip-223.md) `transfer(address, uint, bytes calldata)` function can be declared `payable`, allowing deposits of ether along with tokens sent.
...
If the `tokenReceived` function is declared `payable` in the `IERC223Recipient` - then this would automatically make all the implementations that inherit this interface `payable` too but this approach introduces a security risk of violating a principle of failsafe defaults since the most convenient for the token developer method of handling deposits of ether will lead to a potentially insecure workflow. The `tokenReceived` function may only be `payable` in contracts designed to receive ether deposits. It must not be `payable` in any contracts which are not designed to receive ether deposits as this would prevent accidental deposits of ether.
